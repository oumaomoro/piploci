...
class EconomicNewsFilter:
    ...
    async def is_news_blackout(self, symbol: str) -> Tuple[bool, Optional[str]]:
        ...
        # Map each pair to the currencies that affect it
        PAIR_CURRENCY_MAP = {
            "XAUUSD": ["USD", "XAU"],
            "USDJPY": ["USD", "JPY"],
            "EURUSD": ["EUR", "USD"],
        ...
        if sym in PAIR_CURRENCY_MAP:
            target_currencies = PAIR_CURRENCY_MAP[sym]
        else:
            # Generic fallback: extract 3-char currency codes from the symbol
            if len(sym) >= 6:
                target_currencies = [sym[:3], sym[3:6]]
            elif "USD" in sym:
